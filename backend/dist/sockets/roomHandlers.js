...
function registerRoomHandlers(io, socket) {
    socket.on("join-room", ({ roomCode, username }) => {
        let room = state_1.rooms.get(roomCode);
        if (!room) {
            room = {
                roomCode,
                videoId: "dQw4w9WgXcQ",
            ...
            state_1.rooms.set(roomCode, room);
        }
        const joinMessage = {
        ...
        io.to(roomCode).emit("participants", room.participants);
        users.set(socket.id, {
            socketId: socket.id,
            roomCode,
            username,
    ...
    socket.on("leave-room", () => {
        ...
        if (!user)
            return;
        const room = state_1.rooms.get(user.roomCode);
        if (room) {
            room.participants =
                room.participants.filter(p => p.socketId !== socket.id);
            io.to(user.roomCode).emit("participants", room.participants);
            const leaveMessage = {
                id: Date.now().toString(),
                username: "System",
            ...
            if (room.participants.length === 0) {
                state_1.rooms.delete(user.roomCode);
            }
        }
    ...
    socket.on("video-play", ({ roomCode, currentTime }) => {
        const room = state_1.rooms.get(roomCode);
        if (!room)
            return;
    ...
    socket.on("change-role", ({ roomCode, targetSocketId, role }) => {
        const room = state_1.rooms.get(roomCode);
        if (!room)
            return;
